...
import { 
  ...
  FaUsers, FaSignOutAlt, FaBars, FaTimes, FaTachometerAlt
} from "react-icons/fa";
...
function AdminEditProduct() {
  ...
  const validate = () => {
    let newErr = {};
    if (!form.name.trim()) newErr.name = "Name required";
    if (!form.price || form.price <= 0) newErr.price = "Valid price required";
    if (!form.category.trim()) newErr.category = "Category required";
    if (!form.image.trim()) newErr.image = "Image URL required";
    if (!form.description.trim()) newErr.description = "Description required";
    return newErr;
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#F9F8F6] to-[#EFE9E3] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-[#5D4737]">Loading product...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-8">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-[#D9CFC7] p-6">
              <h1 className="text-3xl font-bold text-[#5D4737] mb-2">Edit Product</h1>
              <p className="text-[#8B7355]">Update product details</p>
            </div>
          </div>
